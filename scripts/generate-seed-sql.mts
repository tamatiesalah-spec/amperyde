...
const frameArr = (v: Component["compatibleFrameTypes"]): string =>
  v == null ? "null" : `'{${v.join(",")}}'`;
const intArr = (v: number[] | undefined): string => (v == null ? "null" : `'{${v.join(",")}}'`);
...
for (const e of [...extras].sort((a, b) => a.sortOrder - b.sortOrder)) {
  lines.push(
    `insert into extras (id, line_id, name, description, price_delta_cents, compatible_frame_types, note, sort_order) values (` +
      `${q(e.id)}, ${q(e.lineId)}, ${q(e.name)}, ${q(e.description)}, ${n(e.priceDeltaCents)}, ${frameArr(e.compatibleFrameTypes)}, ${q(e.note)}, ${n(e.sortOrder)});`,
  );
}
lines.push("");
