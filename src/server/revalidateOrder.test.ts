...
const apexTotal = priceSelection(offRoadCatalog.line, selectionFromComponentIds(apex.componentIds, ctx), ctx, {
...
describe("revalidateOrder (repository-backed)", () => {
  ...
  it("rejects an unknown line without throwing", async () => {
    const r = await revalidateOrder({ lineSlug: "street-legal", componentIds: apex.componentIds, expectedTotalCents: apexTotal });
  ...
  it("rejects a tampered total end-to-end", async () => {
    const r = await revalidateOrder({
      ...
      componentIds: apex.componentIds,
      extraIds: apex.extraIds,
      expectedTotalCents: apexTotal - 25000,
    });
    expect(r.ok).toBe(false);
